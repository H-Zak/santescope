...
export function CommuneEquipments({ data }: CommuneEquipmentsProps) {
  const equipments = [
    { label: "Maison de Santé (MSP)", present: data.msp_presente },
    { label: "Hôpital", present: data.has_hopital },
    { label: "EHPAD", present: data.has_ehpad },
  ];

  const has = equipments.filter((e) => e.present);
  const missing = equipments.filter((e) => !e.present);

  return (
    <div style={{ borderRadius: 10, overflow: "hidden", border: "1px solid #e2e8f0" }}>
      {has.length > 0 && (
        <div style={{ background: "#f0fdf4", padding: "10px 12px", borderBottom: missing.length > 0 ? "1px solid #e2e8f0" : "none" }}>
      ...
      {missing.length > 0 && (
        <div style={{ background: "#fefce8", padding: "10px 12px" }}>
